...
impl OxidizedResourceReader {
    ...
    fn open_resource<'py>(&self, py: Python<'py>, resource: &str) -> PyResult<Bound<'py, PyAny>> {
    ...
    /// FileNotFoundError.
    #[allow(unused)]
    fn resource_path(&self, resource: &Bound<'_, PyAny>) -> PyResult<()> {
    ...
    /// used directly.
    fn contents<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        self.state
            .get_resources_state()
